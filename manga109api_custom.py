...
class Parser(object):
    ...
    def __init__(self, root_dir, authors_list):
        """
        ...
        self.authors = authors_list #  [["id_author", "title"]]
    ...
    def get_annotation(self, book, annotation_type="annotations", separate_by_tag=True):
        ...
        tmp_book=re.sub("_vol.*", "", tmp_book)

        for i in range(len(self.authors)):
            query = self.authors[i][2]
            if re.search(tmp_book, query):
                author = self.authors[i][0]
                break
