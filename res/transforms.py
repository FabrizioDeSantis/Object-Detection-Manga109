    ...
    def forward(
        ...
        if torch.rand(1) < self.p:
            ...
            if target is not None:
                _, _, width = image.size(0), image.size(1), image.size(2)  #F.get_dimensions(image)
...
class RandomIoUCrop(nn.Module):
    ...
    def forward(
        ...
        _, orig_h, orig_w = image.size(0), image.size(1), image.size(2)  #F.get_dimensions(image)
...
class RandomPhotometricDistort(nn.Module):
    ...
    def forward(
        ...
        if r[6] < self.p:
            channels, _, _ = image.size(0), image.size(1), image.size(2)  #F.get_dimensions(image)
...
class ScaleJitter(nn.Module):
    ...
    def forward(
        ...
        _, orig_height, orig_width = image.size(0), image.size(1), image.size(2)  #F.get_dimensions(image)
...
class FixedSizeCrop(nn.Module):
    ...
    def forward(self, img, target=None):
        _, height, width = image.size(0), image.size(1), image.size(2)  #F.get_dimensions(image)
